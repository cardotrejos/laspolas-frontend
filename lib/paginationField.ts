import { FieldMergeFunction, FieldReadFunction, FieldFunctionOptions } from '@apollo/client';
import { KeyArgsFunction, KeySpecifier } from '@apollo/client/cache/inmemory/policies';
import { PAGINATION_QUERY } from '../components/PaginationBrands';
...
export default function paginationField(): 
  FieldPolicy<any, any, FieldFunctionOptions<Record<string, any>>> {
  return {
    ...
    read(existing = [], { args, cache }: { args: any; cache: any; }) {
      ...
      const count = data?._allBrandsMeta?.count;
      const page = skip / first + 1;
      const pages = Math.ceil(count / first);

      // Check if we have existing items
      const items = existing.slice(skip, skip + first).filter((x: unknown) => x);
      // If
      // There are items
